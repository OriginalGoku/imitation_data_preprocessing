...
class DataNormalizer:
    def __init__(self, sentence_length, clip=True, percentage_for_normalization=0.75, clip_max=1.5, clip_min=-0.5,
                 bin_size=0.1):
        # , std_clip_limit=2.5, second_part_std_multiplier=1.5):

        """
        This class normalizes the data. Normalization is made using the following formula: 1. subtract data from min
    ...
    def normalize_tabular_data(self, data_table):
        """
        This function normalizes a data_table based on the rows and not columns
        So each row will be normalized based on its own min/max
        :param data_table:
        :return:
        """


    def normalize_data(self, data_column, verbose=False):
        """
            :param data_column: one column of a dataframe. The function will make a copy of this data in order to avoid modification to the original data
            :param verbose: determines if the function has to print statistics

            """
        data = data_column.copy()
        length = len(data)
